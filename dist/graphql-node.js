...
function GraphQLNode({
  ...
  if (!typeDefs) {
    throw new Error(`GraphQLNode: creating node: '${name}' typeDefs required`);
  }

  try {
    this.typeDefs = parseTypeDefs(typeDefs);
  } catch (error) {
    throw new Error(`GraphQLNode: creating node: '${name}' Error: '${error}'`);
  }

  if (!resolvers) {
    throw new Error(`GraphQLNode: creating node: '${name}' resolvers required`);
  }

  const typeOfResolvers = typeof resolvers;

  if (typeOfResolvers !== "object") {
    throw new Error(`GraphQLNode: expected node: '${name}' resolvers to be of type 'object' but recived '${typeOfResolvers}'`);
  }

  const allowedResolvers = ["Query", "Mutation", "Subscription", "Fields"];
  const notAllowedResolvers = Object.keys(resolvers).filter(key => !allowedResolvers.includes(key));

  if (notAllowedResolvers.length) {
    throw new Error(`GraphQLNode: creating node: '${name}' resolvers recived unexpected properties '[ ${notAllowedResolvers.join(", ")} ]'`);
  }
  ...
  if (enums) {
    if (!Array.isArray(enums)) {
      throw new Error(`GraphQLNode: creating node: '${name}' enums must be of type 'array'`);
    }

    function checkInstanceOfEnum(_enum) {
      if (!(_enum instanceof IdioEnum)) {
        throw new Error(`GraphQLNode: creating node: '${name}' expected enum to be instance of IdioEnum, recived ${JSON.stringify(_enum, undefined, 2)}`);
      }
    }
  ...
  if (nodes) {
    if (!Array.isArray(nodes)) {
      throw new Error(`GraphQLNode: creating node: '${name}' nodes must be of type 'array'`);
    }

    function checkInstanceOfThis(node) {
      if (!(node instanceof GraphQLNode)) {
        throw new Error(`GraphQLNode: creating node: '${name}' expected node to be instance of GraphQLNode, recived: '${JSON.stringify(node, undefined, 2)}'`);
      }
    }
