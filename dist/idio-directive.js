...
function IdioDirective({
  ...
  if (!typeDefs) {
    throw new Error(`IdioDirective: creating Directive: '${name}' typeDefs required`);
  }

  try {
    this.typeDefs = parseTypeDefs(typeDefs);
  } catch (error) {
    throw new Error(`IdioDirective: creating Directive: '${name}' Error: ${error}`);
  }

  if (!resolver) {
    throw new Error(`IdioDirective: creating Directive: '${name}' without a resolver`);
  }
