...
function withBroker(RUNTIME) {
  ...
  return async function execute(document, executionContext = {}) {
    try {
      ...
      let selectedGateway;
      const directives = document.definitions.filter(def => def.kind === "OperationDefinition").flatMap(operation => operation.directives).filter(directive => directive.name.value === "gateway");

      if (directives.length > 1) {
        throw new IdioError(`interservice communication @gateway directive only supported once per document.`);
      }
      ...
      if (directive) {
        ...
        const nameArgument = directive.arguments.find(x => x.name.value === "name");
        ...
        if (!gatewayManagers[directiveGateway]) {
          const foundDirectiveGateways = (await broker.call("$node.list", {
            onlyAvailable: true
          })).filter(_service => {
            const [serviceName, gatewayName] = _service.id.split(":");

            return serviceName === directiveGateway && gatewayName === directiveGateway;
          }).map(({
            id
          }) => id);

          if (foundDirectiveGateways.length) {
            gatewayManagers[directiveGateway] = new ServicesManager(`${foundDirectiveGateways[0]}`, {
              broker,
              hash: directiveGateway
            });
            foundDirectiveGateways.filter(x => x !== directiveGateway).forEach(x => {
        ...
        if (!gatewayManagers[directiveGateway]) {
          throw new IdioError(`cannot reach gateway: '${directiveGateway}'`);
        }

        selectedGateway = await gatewayManagers[directiveGateway].getNextService();
        ...
        selectedGateway = await gatewayManagers[broker.options.gateway].getNextService();

        if (!selectedGateway) {
          throw new IdioError(`cannot reach gateway: '${broker.options.gateway}'`);
        }
      }
      ...
      const {
        data,
        errors
      } = await broker.call(`${selectedGateway}.execute`, {
